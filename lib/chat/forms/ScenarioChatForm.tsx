        ...
        <div>
          ...
          {uploadedFile ? (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.4rem', background: '#f7f6f3', borderRadius: '4px', border: '1px solid #e0e0e0' }}>
              <span style={{ fontSize: '0.65rem', color: '#37352f' }}>📄 {uploadedFile.name}</span>
              <button type="button" onClick={() => { setUploadedFile(null); setContent(''); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#6b6b6b', fontSize: '0.75rem' }}>✕</button>
            </div>
          ) : (
            <div
