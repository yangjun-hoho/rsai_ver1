...
export async function GET(request: NextRequest) {
  ...
  try {
    if (type === 'geocode') {
      ...
      // road 먼저 시도, 실패 시 parcel로 폴백 (V-World는 type=BOTH 미지원)
      const makeUrl = (t: string) =>
        `https://api.vworld.kr/req/address?service=address&request=getCoord&address=${encodeURIComponent(address)}&key=${apiKey}&type=${t}&crs=epsg:4326&format=json`;

      let data = await fetch(makeUrl('road')).then(r => r.json());
      if (data?.response?.status !== 'OK') {
        data = await fetch(makeUrl('parcel')).then(r => r.json());
      }
